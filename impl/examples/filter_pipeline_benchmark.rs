//! Reproducible mutation + tick benchmark for table -> filter -> aggregate.
//! Run: cargo run --release --example filter_pipeline_benchmark -- 10000 100000
//! Construction and warmup are excluded; each sample includes the whole batch.

use livetable::{
    AggregateFunction, AggregateView, ColumnType, ColumnValue, FilterView, Schema, Table,
    TickableTable,
};
